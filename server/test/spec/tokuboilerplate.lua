...
local url = "http://localhost:" .. env.var("PORT") .. "/sync"
local run_id = "spec" .. tostring(utc.time())
...
test("sync endpoint", function ()
  ...
  test("accepts a change set and returns now + changes", function ()
    ...
    err.assert(str.find(body, "\"now\""), "response carries now")
    err.assert(str.find(body, "\"changes\""), "response carries changes")
  end)

  test("previously pushed rows come back for since=0", function ()
    local code, body = post(url .. "?since=0", "[]")
    err.assert(code == 200)
    err.assert(str.find(body, run_id, 1, true), "pushed row echoes back")
  end)
  ...
  test("newer client rows win and filter by since", function ()
    ...
    err.assert(str.find(body2, "edited " .. run_id, 1, true), "newer edit visible past since")
    local code3, body3 = post(url .. "?since=0", change("stale " .. run_id, 0, 0.5))
    err.assert(code3 == 200)
    err.assert(not str.find(body3, "stale " .. run_id, 1, true), "older write must lose")
    err.assert(str.find(body3, "edited " .. run_id, 1, true), "newer body survives")
  end)
